...
{
    #[Route('/qr-codes/{id}', name: 'app_qr_cod', methods: ['GET'])]
public function index(int $id,UrlGeneratorInterface $urlGenerator): Response
{
    ...
    $qrCode = QrCode::create(
        sprintf(
            ...
            $evenement->getNomss(),
            $evenement->getDatee()->format('Y-m-d H:i:s'),
            $reservationUrl = $urlGenerator->generate('app_evenement_show', ['ide' => $evenement->getIde()], UrlGeneratorInterface::ABSOLUTE_URL)
        )
    ...
    $reservationUrl = $urlGenerator->generate('app_evenement_show', ['ide' => $evenement->getIde()]);
